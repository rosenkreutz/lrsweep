...
# Original lrsweep.py by Pseudophed Trismegistus
# Improved by Heinrich Rosen-Kreutz
# lrsweep.py
...
def run():
    ...
    if (args.outputFile):
        ...
        try:
            writeFile = open(args.outputFile, 'w')
        except IOError as err:
            print('I/O Error')
            print('Error(' + format(err.errno) + '): ' + format(err.strerror))
            sys.exit(1)
        ...
        for line in scanFile:
            ...
            print('File Written. Review results in {}'.format(args.outputFile))
            sys.exit(0)
